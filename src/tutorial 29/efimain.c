...
EFI_STATUS efi_main(EFI_HANDLE IH, EFI_SYSTEM_TABLE *ST)
{
	...
	readGDTFile(u"ThatOS64\\gdt.bin");
	...
	SetColor(EFI_YELLOW);
    Print(L"0x");
    itoa(*(unsigned long int*)&bi.GDTFile_Address, GOPINFO, HEX);
    Print(GOPINFO);
	
	SetColor(EFI_LIGHTCYAN);    
	Print(L"\r\nReading GDT File\r\n");
	SetColor(EFI_BROWN);    
	UINT8* test1 = (UINT8*)bi.GDTFile_Address;
	...
	SetColor(EFI_WHITE);
    Print(L"GDT FILE SIZE    : ");
    itoa(*(unsigned long int*)&bi.GDTFile_Size, GOPINFO, DECIMAL);
    SetColor(EFI_YELLOW);
    Print(GOPINFO);
	SetColor(EFI_CYAN);
    Print(L" Bytes\r\n");
