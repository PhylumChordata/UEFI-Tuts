...
void readGDTFile(CHAR16* FileName)
{
    EFI_FILE_PROTOCOL* FileHandle = getFile(FileName);
    if(FileHandle != NULL)
    {
		void* GDTFile_Handle;
		UINT64* GDTFileSize = 0;
		FileHandle->SetPosition(FileHandle, 0xFFFFFFFFFFFFFFFFULL);
		FileHandle->GetPosition(FileHandle, GDTFileSize);
		FileHandle->SetPosition(FileHandle, 0);
		
		if((SystemTable->BootServices->AllocatePool(EfiLoaderData, *GDTFileSize, (void**)&GDTFile_Handle)) != EFI_SUCCESS)
		{
			SetColor(EFI_BROWN);
			Print(L"Allocating Pool for GDT FAILED\r\n");
		}
		
		FileHandle->SetPosition(FileHandle, 0);

        if((FileHandle->Read(FileHandle, GDTFileSize, GDTFile_Handle)) != EFI_SUCCESS)
		{
			SetColor(EFI_BROWN);
			Print(L"Reading GDT File FAILED\r\n");
		}
		
		bi.GDTFile_Size = *GDTFileSize;
		
		bi.GDTFile_Address = GDTFile_Handle;
		
		closeFile(FileHandle);
		
		Print(L"GDT File Loaded\r\n");
    }
}
